' TeamViewer Local Password Switch for TeamViewer 10+
' GSolone 2015 v 0.3
...
if WScript.Arguments.Count = 0 then
   'Richiesta IP o hostname macchina da modificare
	strComputer = InputBox("TeamViewer Local Password Switch" & vbCR & "Lo script permette di abilitare e disabilitare la generazione password di TeamViewer, sia per x86 che x64, rileva automaticamente l'installazione." & vbCR & vbCR & "VA ESEGUITO COME ADMIN LOCALI O DI DOMINIO!" & vbCR & vbCR & "Hostname macchina di destinazione (vuoto o clic su Annulla per uscire dallo script)" & vbCR, "TV Local Passwd Switch", "COMPUTERNAME")
else
...
if LEN(trim(strComputer)) = 0 Then
	...
	strKeyPath64 = "SOFTWARE\Wow6432Node\TeamViewer"
	strValueName64 = "Security_PasswordStrength"
	...
	If IsEmpty(dwValue64) or IsNull(dwValue64) Then
		...
		strKeyPath32 = "SOFTWARE\TeamViewer"
		strValueName32 = "Security_PasswordStrength"
		...
		if dwValue32 = 3 Then
			...
			Wscript.Echo "TeamViewer permette l'accesso esterno su " & strComputer &" (OS 32 bit)"
		else
			dwValue32 = 3
			oReg.SetDWORDValue HKEY_LOCAL_MACHINE,strKeyPath32,strValueName32,dwValue32
			'DEBUG - Togliere commento se necessario
			'Wscript.Echo "Debug scrittura (32 bit): " & dwValue32
			Wscript.Echo "TeamViewer NON permette l'accesso esterno su " & strComputer &" (OS 32 bit)"
		end if
	else
		...
		if dwValue64 = 3 Then
			...
			Wscript.Echo "TeamViewer permette l'accesso esterno su " & strComputer &" (OS 64 bit)"
		else
			dwValue64 = 3
			oReg.SetDWORDValue HKEY_LOCAL_MACHINE,strKeyPath64,strValueName64,dwValue64
			'DEBUG - Togliere commento se necessario
			'Wscript.Echo "Debug scrittura (64 bit): " & dwValue64
			Wscript.Echo "TeamViewer NON permette l'accesso esterno su " & strComputer &" (OS 64 bit)"
		end if
	end if
